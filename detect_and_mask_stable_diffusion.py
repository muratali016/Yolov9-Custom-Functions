            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    if names[int(cls)] in censored:
                        ...
                        # Save the image with transparent region
                        cv2.imwrite("transparent_region_output.png", im0)

                        # Create a black and white mask image
                        black_white_mask = np.zeros((im0.shape[0], im0.shape[1]), dtype=np.uint8)
                        black_white_mask[y1:y2, x1:x2] = 255  # Set the region inside the bounding box to white

                        # Save the black and white mask image
                        mask_path = str(save_dir / 'black_white_mask.png')
                        cv2.imwrite(mask_path, black_white_mask)
